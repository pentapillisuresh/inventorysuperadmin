...
const CreateManager = ({ setCurrentView, editMode = false, managerToEdit = null }) => {
  ...
  useEffect(() => {
    if (editMode && managerToEdit) {
      ...
      setFormData({
        ...
        password: '',
        planType: managerToEdit.planType || (managerToEdit.maxOutlet ? 'Yearly' : 'Monthly'),
        planStartDate: managerToEdit.planStartDate ? managerToEdit.planStartDate.split('T')[0] : '',
      ...
      if (managerToEdit.profileImage || managerToEdit.BusinessImage) {
        setPreviewImage(managerToEdit.profileImage || managerToEdit.BusinessImage);
      }

      if (managerToEdit.businessLogo || managerToEdit.BusinessLogo) {
        setPreviewLogo(managerToEdit.businessLogo || managerToEdit.BusinessLogo);
      }
    }
  }, [editMode, managerToEdit]);
  ...
  const updateUserAPI = async (userData) => {
    try {
      const response = await ApiService.put(`/users/store-managers/${managerToEdit.id}`, userData, {
        headers: {
          Authorization: `Bearer ${clientToken}`,
          'Content-Type': 'application/json',
        },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(`Manager account ${editMode ? 'updated' : 'created'} successfully!`);
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-xl shadow-sm border p-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">
          {editMode ? 'Edit Manager Account' : 'Create New Manager Account'}
        </h1>
        <p className="text-gray-600 mb-8">
          {editMode ? 'Update existing manager account details' : 'Onboard a new manager to the platform'}
        </p>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                ...
                <input
                  type="number"
                  value={formData.limits.maxOutlet}
                  onChange={(e) => handleLimitChange('maxOutlet', e.target.value)}
                  min="1"
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                />
              </div>
